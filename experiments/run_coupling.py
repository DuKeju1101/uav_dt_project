...
def main() -> None:
        ...
        for budget in budget_list:
            for eve_speed in eve_speed_list:
                ...
                for seed_offset in range(int(cfg.get("num_seeds", 5))):
                    ...
                    rows.append(summary)
                    # Export a small set of representative slot traces for debugging.
                    if (
                        budget == DEBUG_BUDGET
    ...
    agg = (
        df.groupby(["periodic_k", "budget", "eve_speed"])[
            [
                "avg_sync_cost",
                "avg_twin_quality",
                "avg_twin_badness",
    ...
    focus_df = agg[(agg["budget"] == focus_budget) & (agg["eve_speed"] == focus_eve_speed)].copy()
    ...
    qmap = agg.copy().astype({"periodic_k": str})
    plot_scatter(
        qmap,
        x="avg_twin_quality",
        y="avg_secrecy_rate",
    ...
    pareto_df = agg.copy().astype({"periodic_k": str})
    plot_pareto(
        pareto_df,
        x="avg_sync_cost",
        y="avg_secrecy_rate",
    ...
    for (budget, eve_speed), g in agg.groupby(["budget", "eve_speed"]):
        tmp = compute_tfscc(g[["periodic_k", "avg_twin_badness", "avg_secrecy_rate"]].copy())
    ...
    if tfscc_rows:
        ...
        df_tfscc = pd.DataFrame(columns=["periodic_k", "avg_twin_badness", "avg_secrecy_rate", "tfscc", "budget", "eve_speed"])
