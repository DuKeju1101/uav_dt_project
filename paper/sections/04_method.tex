...
We implement this idea through a certificate-aware rollout controller. The rollout controller searches over joint synchronization-control actions and evaluates their short-horizon consequences. The empirical certificate supplies a risk penalty and discourages candidate actions whose predicted secrecy margin is not supported by the fitted upper bound.

\subsection{Holdout-fitted secrecy-loss certificate}

The secrecy-loss certificate estimates an upper bound on $L(t)=[\widehat{R}_s(t)-R_s(t)]^+$. The fitted model uses features derived from the digital twin:
\begin{equation}
    \phi(t)=
...
\end{equation}
A calibration residual quantile is added to obtain
\begin{equation}
    \widehat{L}_{\mathrm{ub}}(t)=\widehat{L}(t)+c_{\mathrm{cal}},
\end{equation}
where $c_{\mathrm{cal}}$ is determined on a held-out calibration split. This construction is inspired by distribution-free predictive inference and conformal calibration \cite{lei2018conformal,tibshirani2019covariate}, but the claim made here is deliberately narrower: we validate the bound empirically on held-out simulated trajectories drawn from the evaluated policy and scenario distributions.

For a candidate action, let $\widehat{M}(t)$ be the predicted secrecy margin relative to the outage threshold. The certificate slack is
\begin{equation}
    S_{\mathrm{cert}}(t)=\widehat{M}(t)-\rho-\widehat{L}_{\mathrm{ub}}(t),
\end{equation}
...
\begin{align}
    ...
    &-\lambda_{\mathrm{cert}}[ -S_{\mathrm{cert}}(t)]^+
    -\lambda_{\mathrm{bad}}B_{\mathrm{twin}}(t),
\end{align}
where $\widehat{I}_{\mathrm{out}}(t)$ is the predicted outage indicator, $B_{\mathrm{twin}}(t)$ is a twin-badness term, and the $\lambda$ coefficients are specified in the scenario configuration. The controller then selects
\begin{equation}
...
\begin{algorithm}[t]
\caption{Certificate-aware joint rollout controller}
\label{alg:rollout}
\begin{algorithmic}[1]
...
\For{each candidate action $a_t\in\mathcal{A}_t$}
    ...
    \State Compute predicted secrecy rate, outage indicator, and certificate slack.
    \State Evaluate immediate certificate-aware score.
...
\texttt{periodic} synchronizes at a fixed period and uses greedy communication control. It is simple, stable, and computationally inexpensive. \texttt{security\_risk} triggers synchronization with a weighted score based on age, uncertainty, predicted margin, and budget pressure. \texttt{security\_margin} uses the certificate margin to trigger synchronization when the twin-predicted secrecy margin appears insufficient. \texttt{decoupled} separates synchronization from motion and power control.

The SCA baselines represent local optimization inspired by common nonconvex wireless optimization practice \cite{boyd2004convex}. \texttt{sca\_twin} uses deployable twin information, while \texttt{sca\_oracle} uses true Eve information in the score and is therefore diagnostic. The PPO baseline follows the PPO policy-gradient paradigm \cite{schulman2017ppo}; in this paper it is intentionally described as lightweight because it is trained for 200 episodes rather than extensively tuned.

The strengthening suite includes additional diagnostic variants. \texttt{oracle\_sync} uses the true Eve state inside the rollout score and is treated as an upper-reference method. \texttt{rollout\_fixed\_periodic} keeps rollout motion and power search but replaces adaptive synchronization with fixed periodic synchronization. \texttt{no\_twin} disables effective twin prediction, and \texttt{rollout\_no\_sync} disables synchronization while keeping rollout search. These variants identify whether the proposed gain comes from rollout search alone or from the joint use of synchronization, twin prediction, and certificate-aware scoring.
