...
\begin{itemize}
    ...
    \item certificate cover rate, which measures whether the fitted secrecy-loss upper bound covers realized loss;
    \item runtime per slot, which measures decision-time cost.
\end{itemize}

The final calibrated stress scenario uses $R_{\min}=1.10$. This value was chosen after calibration because the earlier threshold made every evaluated method remain in outage, which removed the ability to compare methods by outage probability. The base and hard scenarios retain their configured thresholds and remain useful for testing whether the controller is competitive under different operating regimes.
